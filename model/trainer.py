...
def Unet_trainer_one_epoch(train_loader: DataLoader, val_loader: DataLoader, train_model: nn.Module, args, criterion, optimizer, pbar):
    ...
    for iteration, (data, label) in enumerate(train_loader):
        ...
        pbar.set_description(
            f"Epoch {epoch+1}/{init_epoch} loss: {loss:.4f} train_acc: {acc:.4f}")
        pbar.update(1)
    pbar.close()
    ...
    with torch.no_grad():
        ...
        for iteration, (data, label) in enumerate(val_loader):
            ...
            if args.loss == "CrossEntropy":
                ...
                t_label = np.transpose(
                    t_label, [0, 3, 1, 2]).argmax(axis=1)
            # update accuracy
            acc = np.sum(t_label == t_pred_label) / \
                np.prod(t_label.shape[1:])
            total_acc += acc
            ...
            if args.log_visual and iteration < max_picture_shown_each_epoch:
                ...
                visual.visualize(np.squeeze(t_pred_label, axis=0), os.path.join(
                    log_path, f"e{epoch}_i{iteration}_pred.png"))
                visual.visualize(np.squeeze(t_label, axis=0), os.path.join(
                    log_path, f"e{epoch}_i{iteration}_label.png"))
                Image.fromarray(np.squeeze(np.uint8(data.detach().numpy()), axis=0).transpose(
                    [1, 2, 0])).save(os.path.join(log_path, f"e{epoch}_i{iteration}_src.png"))
        ...
        if total_acc > best_acc:
            ...
            if args.save_dir == "":
                model_path = os.path.join(
                    log_dir, f"best_acc-{total_acc:.4f}.pt")
            else:
                model_path = os.path.join(
                    log_dir, f"{time_stamp}_epoch-{args.epoch}_lr-{args.lr}_loss-{args.loss}_optim-{args.optimizer}_best_acc-{total_acc:.4f}.pt")
